...
POST /v1/designs/optimal
{
  ...
  "max_iter": 100,
  "seed": 42,
  "region": [[-1.0, -1.0], ...]      # optional (m, k) coded candidate rows;
}                                    # defaults to candidate_grid / mixture_candidates
→ 200
{
  "design": DesignDocument,          # search meta (criterion, score, seed, ...) inside
  "search": {"criterion": "D", "score": 31.2, "d_efficiency": 0.87,
...
{
  "terms": [
    { "term": "temp", "coefficient": 2.0, "effect": 4.0,
      "std_error": 0.31, "t": 6.4, "p": 0.0002,
      "ci_low": 1.28, "ci_high": 2.72 },
    ...
  ],
  "r_squared": 0.981, "adjusted_r2": 0.972, "dof_resid": 5, "mse": 0.42,
  "fitted": [...], "residuals": [...],
...
→ 200
{
  "rows": [ {"term": "temp", "ss": 128.0, "df": 1, "ms": 128.0, "f": 51.2, "p": 0.0008},
            ..., {"term": "Residual", ...}, {"term": "Total", ...} ],
  "lack_of_fit": {"ss_lof": ..., "df_lof": ..., "ss_pe": ..., "df_pe": ...,
                  "f": ..., "p": ...},          # null + "no_pure_error" warning
  "press": 12.4, "predicted_r2": 0.91,          # when replicates are absent
  "warnings": []
}
```
...
{ "design": ..., "response": ..., "model": ...,
  "points": [ {"temp": 55, "time": 4.2, "buffer": "B"}, ... ] }   # natural units
→ 200 { "predictions": [72.1, ...] }
```
...
→ 200
{
  "efficiency": {"d": 0.92, "a": 0.88, "g": 0.85, "i": 0.90},
  "condition_number": 1.7,
  "vif": {"temp": 1.0, "time": 1.02, "temp:time": 1.05},
...
→ 200 { "discrepancy": 0.021, "maximin_distance": 0.48 }
```
...
{ "design": ..., "response": ..., "model": "quadratic" }
→ 200
{ "kind": "maximum",                  # "maximum" | "minimum" | "saddle"
  "natural": {"temp": 61.2, "time": 6.8}, "coded": [0.37, 0.36],
  "response": 84.2, "eigenvalues": [-1.8, -0.4], "warnings": [] }
```

```text
POST /v1/optimize/optimum
{ "design": ..., "response": ..., "model": "quadratic",
  "maximize": true, "bounds": {"temp": [45, 70]} }        # Bounds, see conventions
→ 200
{ "natural": {...}, "coded": [...], "response": 83.1,
  "maximize": true, "at_bound": true, "warnings": [] }
```
...
POST /v1/optimize/desirability
{
  ...
  "goals": [
    { "response": "yield",    "model": "quadratic", "goal": "max",
      "low": 60, "high": 90, "weight": 1.0 },
    { "response": "impurity", "model": "quadratic", "goal": "min",
      "low": 0.5, "high": 3.0 },
...
→ 200
{ "natural": {...}, "coded": [...], "overall": 0.78,
  "responses": {"yield": 82.4, "impurity": 0.9, "cost": 10.2},
  "individual": {"yield": 0.75, "impurity": 0.84, "cost": 0.76},
  "warnings": [] }
```
...
```json
{
  "error": {
    ...
    "errors": ["run 3: missing value for factor 'time'",
               "point_types has 9 entries for 10 runs"]
  }
}
```
